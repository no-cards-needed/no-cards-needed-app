...
export const Stack = ({stack, updateCardPosition, stackRef}) => {
    ...
    return (
        <div className={`cardStack ${stackType === "hand" ? "handStack" : null}`} style={{border: colliding ? '4px solid green' : '4px dashed grey', width, height, opacity: clamp(opacity, 0, 1)}} ref={stackRef}>
            {stackType}
