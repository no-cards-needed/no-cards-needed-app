export const getCardPositionInStack = (
	...
	return controlledStack.current[stackId] ? controlledStack.current[stackId].findIndex((card) => card === cardId) < 0 ? 0 : controlledStack.current[stackId].findIndex((card) => card === cardId) : 0;
