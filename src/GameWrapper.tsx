...
function translateCardName(symbol: string) {
  if (symbol.includes("H")) {
    color = "♥";
  } else if (symbol.includes("D")) {
    color = "♦";
...
export const GameWrapper = ({ app }: { app: any }) => {
      ...
      if (snapshot.val() && Object.keys(snapshot.val()).length === 1) {
        ...
        if (
          ...
          set(gameStatusRef.current, gameStatus)
            .then(() =>
              console.log("👁️ [gamewrapper] game status set", gameStatus)
            )
            .catch((error) =>
              console.log(
  ...
  const setGameStatus = (updatedGameStatus: GameStatus) => {
    ...
    set(gameStatusRef.current, updatedGameStatus)
      ...
      .catch((e) =>
        console.log("👁️ [gamewrapper] Encountered error setting game status", e)
      );
  };
  ...
  const setCard = (card: Card, cardId: number, timestamp: number) => {
    ...
    if (
      ...
      set(cardRef, _card)
        ...
        .catch((e) =>
          console.log("👁️ [gamewrapper] Encountered error setting the card", e)
        );
      ...
      const stack =
        tableStacksState.get(card.onStack) || handStacksState.get(card.onStack);
  ...
  const setTableStack = (stack: Stack, stackId: number, timestamp: number) => {
    ...
    if (
      ...
      set(currentTableStackRef, _stack)
        ...
        .catch((e) =>
          console.log("👁️ [gamewrapper] Encountered error setting the stack", e)
        );
    } else {
      updateGameStatusTimestamp();
    }
  ...
  const startGame = () => {
    ...
    set(cardsRef.current, distributor.cards)
      ...
      .catch((error) =>
        console.log("👁️ [gamewrapper] Encountered error setting cards", error)
      );
    set(tableStacksRef.current, convertStacksMapToObject(distributor.stacks))
      .then(() => console.log("👁️ [gamewrapper] stacks set"))
      .catch((error) =>
        console.log("👁️ [gamewrapper] Encountered error setting stacks", error)
      );
