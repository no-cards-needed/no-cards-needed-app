...
export const GameWrapper = ({ app }: { app: any }) => {
	...
	const shuffleTableStack = (stackId: number | string, timestamp: number) => {
		const currentTableStackRef = ref(
			getDatabase(app.current),
			`game/${gameId}/tableStacks/${stackId}`
		)
		...
		const stack = tableStacksState.get(stackId)
		const cards = Array.from(stack.cards)
		const shuffledCards = shuffle(cards)

		console.log("CARDS POST SHUFFLE", new Set(shuffledCards))
		const _stack: Stack = { ...stack, cards: new Set(shuffledCards) }

		setTableStack(_stack, stackId as number, timestamp)
		const newGameLog: GameLog = gameLogState
			? [
					...gameLogState,
					{
		...
		<>
			{loading ? (
				...
				<PlayingGame
					...
					shuffleTableStack={shuffleTableStack}
					players={allPlayers}
					avatars={avatars}
				/>
			)}
