...
export const GameWrapper = ({ app }: { app: any }) => {
	...
	const initGame = () => {
		...
		// Game Logging
		gameLogRef.current = ref(getDatabase(app.current), `game/${gameId}/gameLog/`)
	...
	const setCard = (card: Card, cardId: number, timestamp: number) => {
		...
		if (gameStatusState.timestamp && timestamp > (gameStatusState.timestamp as number)) {
			...
			const stack = tableStacksState.get(card.onStack) || handStacksState.get(card.onStack)
			const wasStackVisible = stack.stackType === "front" || stack.stackType === "open"
			const wasStackHand = stack.stackType === "hand"

			const logMessage = `${allPlayers.get(userId).name} ${wasStackHand ? "took" : "played"} ${
				wasStackVisible ? translateCardName(card.symbol) : "a card"
			}`

			const handStack = handStacksState.get(userId)
			!wasStackHand ? handStack.cards.delete(card.cardId) : handStack.cards.add(card.cardId)
